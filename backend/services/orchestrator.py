    ...
    async def _run_agent(
        ...
        if use_deep_research and role == "paper_writer":
            try:
                ...
                # Assuming hardcode key logic is inside client or passed here. 
                # Service relies on client logic.
                deep_client = GeminiDeepResearchClient()
                
                print(f"[Orchestrator] Using Gemini Deep Research for {role}...")
                
                # Deep Research Output might be raw, we might need to wrap it.
                # However, the user said "invoke deep research function".
                # The task is the prompt.
                # Gemini Deep Research takes time.
                response_text = await deep_client.run_research_task(task)
                
                # Add signature
                response = response_text + f"\n\n-- Generated by Gemini Deep Research & {self.client.model} --"
            except Exception as e:
                print(f"[Orchestrator] Deep Research failed for {role}: {e}. Falling back.")
